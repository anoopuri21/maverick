# 06 · Program Detail Page Content: BBA in Project Management (v5)

**Page:** /programs/bba-project-management
**Awarding university:** Rushford Business School, Switzerland · dual certified with Maverick Business Academy, London
**Category:** BBA · **Level:** Bachelor's · **Duration:** 20 to 24 months, fully online
**Voice pass:** v5 maximally-human pattern (written from client source content, proofed, quality-lifted)
**Status:** CONTENT READY · excluded sections (per client list) not written: recognition, accreditation, university block, success stories, reviews, network, faculty voice, FAQ, enquire, CTA
**Sources on file:** client BBA Project Management source document (curriculum, careers, assessment, duration), Dubai Land Department 2025 record figures (fam Properties analysis), UAE Net Zero 2050 (Reuters), approved bachelors landing page

---

## SEO

### meta_title
BBA Project Management Online | Maverick Business Academy London

### meta_description
Study BBA Project Management online and develop skills in project planning, scheduling, risk management, quality, procurement, agile project management and leadership.

### url_slug
/programs/bba-project-management

---

## 1. HERO

### title
BBA in Project Management

### level
Bachelor's

### short_description
Build the business, leadership, and project-management skills to plan, execute, monitor, and deliver projects successfully across industries. Rushford Business School's BBA runs 20 to 24 months, fully online.

### duration
20 to 24 months online

### highlights (Quick Highlights)
Rushford Business School | 20 to 24 months online | Project management focus | Career-focused program | Flexible learning | Scholarship available

---

## 2. PROGRAM AT A GLANCE (snapshot)

Degree Award | Bachelor of Business Administration (BBA)
Awarding University | Rushford Business School, Switzerland
Specialization | Project Management
Duration | 20 to 24 months
Assessments | Assignments, projects, case studies & applied assessments
Focus | Project Planning, Risk, Quality, Leadership & Project Delivery

### note
Program duration and entry pathway may vary depending on your previous academic qualifications, professional background, and approved progression pathway. Admissions confirms everything in writing, fees included, before any payment.

---

## 3. PROGRAM OVERVIEW (description)

### copy
Projects drive change, innovation, and growth across almost every industry. From construction and technology to healthcare, logistics, engineering, finance, and government, organizations need professionals who can manage time, cost, quality, people, risk, and resources effectively. The BBA in Project Management develops strong business-management knowledge together with specialised project-management capabilities.

Students build a foundation in leadership, financial awareness, business operations, decision-making, customer management, and organisational development, while gaining focused knowledge in project planning, scheduling, risk management, quality, procurement, agile methods, and project leadership. It suits aspiring project coordinators, working professionals who already contribute to projects, and people in construction, engineering, IT, and operations who want the management profile to match the work they do.

---

## 4. WHY CHOOSE THIS PROGRAM (benefits)

**01 · Business and project management, combined**
Broad business knowledge with specialised project-management expertise, which is the profile PMO roles actually ask for.

**02 · Plan projects properly**
How projects are defined, structured, scheduled, and managed from initiation to completion.

**03 · Risk and quality skills**
How to identify risks, maintain quality, and manage project challenges when the plan meets reality.

**04 · Leadership capabilities**
The communication, coordination, and team-management skills required to lead project teams.

**05 · Modern delivery methods**
Traditional and agile approaches used to manage projects in changing business environments, Scrum included.

**06 · Study around your career**
Flexible online learning allows working professionals to continue their education while managing professional responsibilities.

Closing line: Rushford Business School, Switzerland, awards the degree. The projects you deliver on time and on budget afterwards are the references that count.

---

## 5. LEARNING OUTCOMES (learning)

01 Understand fundamental business and management principles
02 Plan and organise projects effectively
03 Understand project scope, time, and cost management
04 Identify and evaluate project risks
05 Understand quality-management principles
06 Apply procurement and resource-management concepts
07 Lead and coordinate project teams
08 Understand agile and adaptive project-management methods
09 Analyse project performance and make informed decisions
10 Apply strategic thinking to project and organisational challenges

---

## 6. CAREER OPPORTUNITIES (careers)

Project Coordinator
Junior Project Manager
Project Management Executive
Project Support Officer
Project Administrator
Project Planning Coordinator
PMO Coordinator
Operations Coordinator
Project Control Assistant
Project Scheduler
Business Project Executive
Project Procurement Coordinator
Risk & Compliance Coordinator
Quality Coordinator
Construction Project Coordinator
IT Project Coordinator
Events Project Coordinator
Logistics Project Coordinator
Business Operations Executive
Program Support Executive

Roles come from the program's official career outcomes. Exact opportunities depend on experience, industry, professional skills, and employer requirements.

---

## 7. PROGRAM STRUCTURE (structure)

### heading
Your journey, year by year

### intro
Core business and management foundations first, then advanced business subjects and specialised project management study.

### stages

**Year 1 · Business and management foundations**
- Communications in Organizations
- Leadership and the Organization
- Financial Awareness
- Managing Change
- Business Operations
- Developing Teams

**Year 2 · Advanced business and management**
- Responding to the Changing Business Environment
- Effective Decision Making
- Business Development
- Business Models & Growing Organizations
- Customer Management
- Risk Management & Organizations

**Project management specialization**
- Project Planning, Risk and Quality Management
- Agile Project Management and Scrum
- Project Finance, Procurement and Stakeholder Management

**Program focus areas**
- Business management, project planning, scheduling, risk management, quality management, procurement, project leadership, agile project management, stakeholder management, and strategic decision-making

---

## 8. WHY STUDY THROUGH MAVERICK (support)

Dedicated academic support | Guidance to help you understand academic requirements and maintain progress
Experienced faculty | Academics and professionals who bring practical business and project-management perspectives
Student success team | Ongoing support from enrollment through program completion
Assignment support | Guidance on assessment requirements, academic expectations, and project-based assignments
Career guidance | Support in connecting your qualification with professional development goals
Documentation assistance | Administrative help throughout your student journey, including graduation procedures

---

## 9. WHY GCC STUDENTS CHOOSE THIS COURSE (gcc_reasons)

### gcc_heading
Why GCC professionals choose this course

### cards

**01 · The region runs on projects**
The GCC continues to invest in infrastructure, technology, tourism, real estate, energy, and large-scale transformation projects, with mega-project activity running strong toward 2029.

**02 · Demand crosses every industry**
Construction, engineering, technology, aviation, logistics, healthcare, energy, banking, government, events: project-management skills are relevant across all of them.

**03 · Property alone is a project pipeline**
Dubai recorded AED 686.8 billion in property sales across 215,736 transactions in 2025, per Dubai Land Department data, and every development behind those numbers is a project needing coordination.

**04 · Energy transition adds another layer**
The UAE's net-zero plan commits AED 600 billion to clean and renewable energy through 2050, and those programs are delivered through project teams.

**05 · No career break required**
Study from the UAE, Saudi Arabia, Qatar, Oman, Bahrain, or Kuwait while continuing your career, with the degree moving around your week.

**06 · Progression stays open**
After the bachelor's, graduates may explore eligible postgraduate pathways including an MBA or master's degree, subject to admission requirements.

Honest take: project management certifies with experience, and professional certifications like the PMP will come later in your career. This degree builds the business and leadership base those certifications assume, which is the right order to do it in.

---

## 10. FEES & SCHOLARSHIPS (fees)

Program Fees | Vary with the intake, approved entry route, and scholarship opportunities
Scholarships | Opportunities vary by intake; admissions confirms the latest position
Payment Options | Registration fee, initial payment, and monthly installments itemised in the full structure
Written Breakdown | Complete fee breakdown from the admissions team before any payment
Intake Offer | Current intake offers confirmed per intake, in writing

### request block
Fee structures vary with the intake, approved entry route, and available scholarship opportunities. Speak with the admissions team for a personalised breakdown based on your academic profile, intake, and study pathway. No commitment required.

---

## BUILD NOTES (internal, not page copy)

- Built from the client source document "BBA Project Management - Rushford Business School.docx": duration (20-24 months stated), curriculum (Year 1 core 6, Year 2 core 6, three specialization modules), assessment types, career roles (20), learning outcomes (10) all taken from it.
- PROOF FIX: source specialization module titles carried inconsistent casing and a typo ("stake holder management"); written as "Project Planning, Risk and Quality Management", "Agile Project Management and Scrum", "Project Finance, Procurement and Stakeholder Management". Confirm with client.
- Excluded per client section list: Institution/Accreditation & Recognition blocks, Student Success Stories, Our Network, Faculty Voice, FAQ, Enquire form, final CTA.
- GCC market data: Dubai Land Department 2025 record (AED 686.8B, 215,736 transactions, via fam Properties analysis); UAE Net Zero 2050 (AED 600B, Reuters); mega-projects-to-2029 framing used earlier in this project. All softenable if legal wants.
- Honest-take note on PMP positioning is advisory, not a claim about the degree. Soften if client prefers.
- Fees section carries no figures (source gives none).
- Sign-off needed: exact fee per intake, scholarship rules, September 2026 intake dates, module-title corrections confirmation.
