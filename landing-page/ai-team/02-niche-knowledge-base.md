# 02 — Maverick Niche Knowledge Base
**Mandatory reading for every role before every task. No exceptions.**
**Updated:** 2026-09-14 · **Guardian:** T6 (Education Niche SME) · **Full sources:** `../bachelors/03-research-bachelors-gcc.md`, `../bachelors/04-plan-research-verification.md`

This brief makes every team member a working expert in the Maverick niche:
UK and partner-university degrees sold to students living in the Gulf, delivered
online or hybrid, with a real office in Sharjah. Read it once per project,
re-read the sections your task touches.

---

## 1. Who Maverick is

- **Maverick Business Academy.** London-headquartered (Ruislip) since **2012**. UAE office at **Robot Park Tower, 2nd Floor, Sharjah**.
- What it sells: undergraduate and postgraduate routes. **BBA** from Rushford Business School, Switzerland. **BSc** from Girne American University, North Cyprus. **BA (Hons) Global Business** from the University of the West of Scotland, UK. **Top-up routes** via Ofqual/QUALIFI diploma into UK honours. Plus MBA/MSc and Level 3–8 Ofqual diplomas.
- The promise in one line: a real degree from a named university, without a student visa, without leaving the Gulf, with AED instalments and a human counsellor one call away.
- Conversion rails: 5-field enquiry form (counsellor call within 1 business day), WhatsApp (the Gulf's default channel), fee-plan download.

## 2. The market (facts carry source + date; never quote without them)

| # | Fact | Source (date) |
|---|---|---|
| M1 | UAE HE institutions admitted **57,035 new students in 2024–25**, +13% YoY. Highest intake in a decade | MoHESR via The National (2025-11) |
| M2 | Dubai private HE: **42,026 students, +20% YoY**; international +29% → **35% of the student body**; 37 international campuses | KHDA via The National (2025-11) |
| M3 | Indian students ≈ **42% of Dubai's international intake**, concentrated in business & technology | EducationFair UAE market report (2026-05) |
| M4 | Business & economics is the **most-pursued field** in the UAE | UNESCO data via EducationFair (2026-05) |
| M5 | Federal HE enrolment **182,083 (2024)**, +33% vs 2020 | Federal statistics via Ken Research (2025) |
| M6 | **153 licensed HE institutions** in the UAE | MoHESR licence list via EduPrime (2026-07) |
| M7 | Dubai Education 2033: target **50% international share by 2033** | Education 2033 strategy |
| M9 | **400+ listed bachelor's programmes** in the UAE; Sharjah alone: 40 (11 online) | bachelorsportal (2026), educations.com (2026-09) |
| M10 | Campus tuition band **AED 35k–65k/yr**; premium BBA AED 33k–110k; S P Jain BBA online AED 100k/4yr | Fateh (2026-01), coursetakers.ae (2026-04), educations.com (2026-09) |

**Dropped facts (never reuse):** "237,000+ international students" (EduPrime; conflicts with official figures). The Mar-2025 "18 credit-hours/semester" online cap (superseded by the 2025 easing; quote no credit-cap number).

## 3. Regulation: the locked phrasings (R-pack)

Recognition is the trust spine of every page. Use these exact framings, source-dated:

| ID | Locked fact | Copy rule |
|---|---|---|
| R1 | UAE MoE began recognising online degrees in **2023** (Engineering, Medicine, Law excluded) | Background only; lead with R2 |
| R2 | **10 Mar 2025, MoHESR policy:** conditional recognition of distance/online/open/correspondence degrees; ministerial committee; fees Dh100 (bachelor); verification via Dataflow or QuadraBay; ~30 working days; 3-month appeal | **The primary recognition fact on every page.** Always sourced, always dated |
| R3 | 2025 (H2): MoHESR eased the earlier online-credit caps | Phrase as "under the current 2025 MoHESR framework". **No credit-cap numbers** |
| R4 | Private sector: accredited university + verification is generally enough. The Certificate of Recognition (equivalency) is for government roles, regulated professions, some visa categories | Use: *"Recognised for private-sector use; MoHESR recognition route available — confirm your category with an advisor."* (quoted phrasing, verbatim) |
| R5 | MOHRE accepts accredited online/distance degrees for work permits; a bachelor's is expected for most AED 4,000–8,000/month professional bands | Career framing only; no salary promises |
| R6 | UK degree attestation route: FCDO apostille → UAE Embassy/MOFA; Dataflow/QuadraBay first | Offer "we guide the full route". A counselling hook, never legal advice |

**Never write:** "visa guaranteed", "recognised for all purposes", "government-approved job guarantee", or any equivalency promise.

## 4. Partners and accreditations (verified 2026-09; use only these)

| Partner | Award | Accreditations we may state |
|---|---|---|
| Rushford Business School (Lucerne, CH) | BBA ×7 specialisations; MBA/MSc | **EduQua, IACBE, QS Stars 5-star (teaching & online)**; dual certification with Maverick London (live on RBS's site) |
| Girne American University (North Cyprus) | BSc ×10 programmes | **YÖDAK (TRNC) + YÖK (Turkey), IACBE, ECBE, ASIIN, UNWTO TedQual, NARIC-UK** |
| University of the West of Scotland (UK) | BA (Hons) Global Business | UK university (QAA-regulated). **Programme-level details: client to confirm** |
| QUALIFI / Gatehouse (Ofqual, UK) | Level 3/5/7 diplomas | **Ofqual-regulated.** Diplomas, not degrees. Top-up awarding body (UoG/UCLan): **client to confirm** |

**Excluded claim:** RBS ACBSP/AACSB membership. It appears only on agent pages, not on RBS's own site. Do not use it.
**Precision rule:** "UK-recognised" must mean, per occurrence, either (i) awarded by a UK university, or (ii) Ofqual-regulated. Diplomas ≠ degrees. Never blur them.

## 5. The three personas

| Persona | Who | Core fear | Channels | Decision driver |
|---|---|---|---|---|
| **P1: New graduate + family** | 18–22, just finished 12th; parent co-decides and pays | Is it real, recognised, worth it? What will employers say? | WhatsApp, Facebook, school networks | Recognition + cost + safety |
| **P2: Working professional** | 22–32, earning in the Gulf | Losing salary/time to a campus grind | LinkedIn, Google | Fits around the job; no visa |
| **P3: Top-up candidate** | Holds diploma/HND/credits | Will credits count? How long? | Google exact-match | Fastest legitimate route to a named degree |

**Two-audience tone rule:** every major section must pass both ears. The student hears ambition and flexibility. The parent hears recognition, safety and cost control.

## 6. The competitive landscape (2026-09)

- **Campus premium.** S P Jain (BBA online AED 100k/4yr), Middlesex Dubai, UOWD. AED 35k–65k/yr, plus visa, relocation, 3–4 years.
- **Online agents.** Global Learners (11 online bachelors, Sharjah), Westford (LJMU-awarded), LUBM, King's Business School (12-month BBA). Awarding bodies buried; no physical academy behind them.
- **Fast-track mills.** Ascencia (1-yr BBA), Study Wink (AED 7,999), Sama (AED 3,500), Tamkeen (AED 22,500/12mo). The recognition risk is the story. We criticise the *category*, sourced to R2, never by name.
- **Agent content farms.** Fateh, AECC, bradfordia (running the exact K2/K5 phrases as a cluster), directories (educations.com, coursetakers.ae, bachelorsportal). They own the SERP, not the student.
- **The gap Maverick owns.** Nobody credible combines named awarding universities + no student visa + weekend-tolerant delivery + AED instalments + a physical Gulf office + parent-safe recognition content. That is every page's spine.

## 7. Glossary (the team's shared vocabulary)

- **Top-up.** Converting an existing diploma/HND/credits into a full honours degree, usually in one final year. The fastest *legitimate* route to degree level.
- **HND.** Higher National Diploma (UK). Credit-bearing; eligible for top-up.
- **Ofqual.** The UK qualifications regulator. Regulates diplomas (QUALIFI, Gatehouse), not degrees.
- **QUALIFI.** Ofqual-regulated UK awarding body; its Level 5/7 diplomas feed top-up degrees.
- **MoHESR.** UAE Ministry of Higher Education & Scientific Research; runs the 2025 foreign-degree recognition policy (R2).
- **MoE.** UAE Ministry of Education; began online-degree recognition in 2023 (R1).
- **MOHRE.** UAE Ministry of Human Resources & Emiratisation; accepts accredited online degrees for private-sector work permits (R5).
- **Attestation.** Legalising a degree for use in the UAE: FCDO apostille (UK) → UAE Embassy/MOFA; Dataflow/QuadraBay verification first.
- **Equivalency (Certificate of Recognition).** Formal MoHESR document needed for government roles, regulated professions, some visa categories. Not needed for most private-sector jobs.
- **Rotational intakes.** Rolling start dates through the year, not one September intake.
- **Weekend-tolerant.** Timetables that survive a full-time Gulf job.
- **AED-first.** Prices in dirhams; INR only as a dated parenthetical for the Indian-expat rail.

## 8. Claim sources: what may enter any Maverick text

Only three kinds of claim are allowed (the whitelist; full policy in `guidelines/G06`):
1. Facts from the client's own catalogue (`uploads/listing.pdf`) and client-confirmed inputs.
2. Public facts with a named source and a date (the M- and R-packs above, competitor published prices).
3. Client-verified statistics with a source.

Everything else stays out. No "best", "top", "#1", "guaranteed", "100%". No salary figures. No invented testimonials.

## 9. House style facts (used across all deliverables)

- UK spellings: *programme(s)*, *recognised*, *instalment*.
- "bachelor's degree" (apostrophe, noun) vs "bachelors" (no apostrophe, attributive: bachelors programmes, bachelors landing page).
- Brand colours if design-adjacent: navy `#071444`, red `#b20202`, warm paper `#f5f0eb`.
- Cities named exactly: Dubai · Sharjah · Abu Dhabi. "GCC" appears at least twice per page-level asset.
