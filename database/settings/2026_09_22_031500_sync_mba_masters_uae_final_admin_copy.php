...
return new class extends SettingsMigration
{
    public function up(): void
    {
        ...
        $this->migrator->update('mba_masters_hero.form_title', fn () => 'Send me the program guide');

        $this->migrator->update(
            'mba_masters_trust.label',
            fn () => 'Rated 4.9 out of 5 by people who finished the degree'
        );
        $this->migrator->update('mba_masters_trust.quote', fn ($quote) => $this->stripTrustpilot((string) $quote));
        $this->migrator->update('mba_masters_trust.stats', function ($stats) {
            $stats = $this->rows($stats);
            foreach ($stats as &$stat) {
                $label = $this->stripTrustpilot((string) ($stat['label'] ?? ''));
                ...
                if (str_starts_with(mb_strtolower($label), 'online')) {
                    $label = 'Online. No visa. No relocation.';
                }
                $stat['label'] = $label;
            }
            unset($stat);
        ...
        $this->migrator->update(
            ...
            fn () => 'A Master\'s with Maverick is not a break from your life. It is a practical way to work toward a bigger role and a clearer salary conversation, while your salary keeps coming in.'
        );
        $this->migrator->update('mba_masters_why.chapters', fn () => [
            [
                'title' => 'Learn without pausing your salary',
                'text' => 'Campus programs ask you to quit, relocate, or wait for Saturday. This one runs live on Tuesday and Thursday evenings, from where you already are. You can use what you learned the next morning. That is what a flexible MBA is for: your salary stays in place.',
                'anchor' => null,
            ],
            [
            ...
            [
                ...
                'text' => 'A campus MBA in cities across the UAE usually costs AED 80,000 to 200,000. Most programs on this page total AED 16,000 to 40,000, paid monthly in dirhams and timed to your salary date. The degree fits your cash flow from the first month.',
                'anchor' => '#mlp-fees',
            ],
            [
                'title' => 'A recognized route, explained honestly',
                'text' => 'You study 100% online from the UAE. The university awards the degree, and your advisor explains in writing how employers here usually read that award, before you pay anything. If you have a question about recognition, we answer it in plain language. We would rather lose an enrollment than leave you with a surprise later.',
                'anchor' => '#mlp-faq',
            ],
            [
        ...
        $this->migrator->update('mba_masters_journey.steps', fn () => [
            ...
            [
                ...
                'text' => 'Within 24 hours we confirm, in writing, the admission requirements, the right program, the total fee, and the intake dates.',
            ],
            [
        ...
        $this->migrator->update('mba_masters_class.audience', fn () => '');
        $this->migrator->update('mba_masters_class.metrics', function ($metrics) {
            ...
            $metrics = array_slice($metrics, 0, count($labels));
        ...
        $this->migrator->update(
            ...
            fn () => 'There are no hidden charges, and we do not print outdated numbers. Across these programs, the total is usually between AED 16,000 and 40,000, about a third of a campus MBA in Dubai. We confirm your exact figure in writing before you pay a dirham.'
        );
        $this->migrator->update(
            'mba_masters_fees.note',
            fn () => 'Fees change with the program and the intake. Written confirmation always comes before any payment.'
        );
        $this->migrator->update('mba_masters_fees.rows', function ($rows) {
            ...
            foreach ($rows as &$row) {
                ...
                if (str_starts_with($program, 'Global MBA') || str_starts_with($program, 'Dual MBA')) {
                    $row['program'] = 'Dual MBA, UCA with Rushford';
                }
                if (in_array($row['mode'] ?? '', ['Online', '100% Online'], true)) {
                    $row['mode'] = '100% Online';
                }
                if (str_contains(mb_strtolower((string) ($row['duration'] ?? '')), 'confirmed')) {
                    $row['duration'] = 'Confirmed for each program';
                }
            }
            unset($row);
        ...
        $this->migrator->update('mba_masters_fees.blocks', fn () => [
            ...
            [
                ...
                'text' => 'Your fee covers full tuition and the university award, all study materials and portal access, session recordings, advisor support from Sharjah on Gulf time, plus orientation, enrollment, and student services.',
            ],
            [
        ...
        $this->migrator->update(
            ...
            fn () => 'Every Master\'s on this page is awarded by one of the universities below. You apply once, study 100% online, and graduate with that university\'s name on the certificate. One application covers admission, enrollment, and your study plan.'
        );
        $this->migrator->update(
        ...
        $this->migrator->update('mba_masters_partners.checklist', fn () => [
            ...
            ['text' => 'Accreditation status is confirmed in writing before you enroll'],
        ...
        $this->migrator->update('mba_masters_faq.items', fn () => [
            [
                ...
                'answer' => 'Most programs ask for a bachelor\'s degree and two to three years of work experience. None of them require a GMAT. If your background looks different, send your CV anyway. Our advisors map entry routes for experienced professionals every week, and they confirm eligibility in writing.',
            ],
            [
                'question' => 'Are these degrees recognized in the UAE?',
                'answer' => 'You study 100% online from the UAE, and the university awards the degree. The certificate does not mention online study. Employers across the Gulf usually look at the awarding university, which is why we confirm those details in writing before you enroll. If you want to talk through how your employer is likely to read the certificate, an advisor will do that with you at no charge.',
            ],
            [
                'question' => 'How much does an MBA cost in the UAE?',
                'answer' => 'Most programs on this page cost between AED 16,000 and 40,000, paid in instalments. A campus MBA in Dubai typically costs about three times that. Your advisor confirms your exact total in writing before you pay.',
            ],
            [
            ...
            [
                ...
                'answer' => 'Most do, and many sponsor the fees. Employers look at the university that awards the degree, and the certificate does not mention that you studied online. We confirm the award details in writing before you enroll. If HR needs sponsorship paperwork, we prepare it with you.',
            ],
            [
        ...
        $this->migrator->update('mba_masters_final.form_title', fn () => 'Send me the program guide');
        $this->migrator->update(
    ...
    private function stripTrustpilot(string $text): string
    {
