...
7. Migration mapping store: options — (a) nayi table `media_migration_map`, (b) JSON state file in `storage/`. Recommendation: **table** (queryable + resumable + auditable).
8. `.env.example` update — `CLOUDINARY_ENV_FOLDER=false` + `CLOUDINARY_DISK_ENV=shared` explicit lines (docs-only change).
